...
const Updatedetails:React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialVersion = async () => {
      ...
      try {
        setLoading(true)
        const initialVersionString = await appManagementService.getLatestVersion(appDetails);
        console.log(initialVersionString);
        const initialVersionArray = initialVersionString.split('.').map(Number);
  ...
  const handleApprovalClick = async () => {
    ...
    try {
      ...
      const response = await appManagementService.updateAppdetails(approvalData);
      ...
      handleSnackbarOpen('error', error.response?.data || 'An error occurred');
    }
    finally{
      setLoading(false);
    }
