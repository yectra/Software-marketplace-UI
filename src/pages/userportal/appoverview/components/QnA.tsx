...
import { Answerparams, QuestionAnswerResponse, QuestionParams } from '@/pages/userportal/appoverview/models';
...
const QnA: React.FC<QnAProps> = () => {
  ...
  const fetchQnA = async () => {
    try {
      setLoading(true);
      const response = await getApplications.getQnAforApps(appId);
  ...
  const handleSubmitQuestion = async () => {
    ...
    try {
      setLoading(true);
      const response = await getApplications.addQuestionsForApps(appId, newQuestionDetails);
      console.log(response);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      setLoading(true);
      const response = await getApplications.addAnswerForQuestions(appId, selectedQuestionId, answerDetails);
      console.log(response);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <Box sx={{ p: 3 }}>
          ...
          <Grid item xs={12} key={q.questionId}>
            <Card>
              <CardContent sx={{ display: 'flex' }}>
                <Avatar sx={{ width: 56, height: 56, mr: 2, bgcolor: '#0C9DBD' }}>
                  {q.userName.charAt(0)}
                </Avatar>
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                    {q.answers.length > 0 ? q.answers[0].answerText : 'No answers yet'}
                  </Typography>
                  {selectedQuestionId === q.questionId ? (
                    <Box sx={{ mt: 2 }}>
                      <TextField
                        ...
                        sx={{ mb: 1 }}
                      />
                      <Button onClick={handleSubmitAnswer} sx={{ bgcolor: '#0C9DBD', color: 'white' }}>
                        Send
                      </Button>
                    </Box>
                  ) : (
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>Please enter your question about the app below.</DialogContentText>
          <TextField
            autoFocus
            label="Your Question"
            fullWidth
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmitQuestion} sx={{ color: '#0C9DBD' }}>
            Submit
          </Button>
        </DialogActions>
      </Dialog>
