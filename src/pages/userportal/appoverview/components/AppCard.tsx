...
import { CloseButton, IframeContainer, StyledModal } from '@/common/UI/StyledElements';
...
const AppCard :React.FC<AppProps> = () => {
  ...
  useEffect(() => {
    ...
    //   if (appId) {
    //     const response = await getApplications.getApplicationdetails(appId);
    //     setAppDetails(response);
    //     console.log(response);
    //   }
    // };
    getApplications.getApplicationdetails(appId)
    .then((response)=>
      { setAppDetails(response)
  })
    ...
    <Box>
      <Box sx={{display: "flex", alignItems: "center", gap: 2, mt: 2}}>
        <Box sx={{
          ...
          {/* <Typography variant="body1">{getInitials({appName: appName})}</Typography> */}
          <img
            src={appDetails?.icon}
            alt="app logo"
            style={{
        ...
        <Box>
          ...
          <Box sx={{display: "flex", gap: 1}}>
            ...
            <Rating name="read-only" value={appDetails?.overallRating || 0} readOnly sx={{color: "#0C9DBD"}}/>
